...
db.bind(collectionName, {
  ...
  , getByNameOrEmail: function(name, email, fn){
        this.find({ $or: [ {name_lower: name.toLowerCase()}, {email: email.toLowerCase()} ] }, {'tickets': 0}).limit(1).toArray(function(err, user){
            fn(err, user && user[0]);
        });
    }
